...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild2 {
     ...
     * <p>
     *     滚动期间适配器更改可能会触发崩溃，因为滚动假设没有数据更改但数据实际更改。
     ...
     * 因为recyclerview中含有不在UI中显示的虚拟view,所以焦点查找策略同其他的viewgroup不同
     * <p>
     ...
     * 调用{@link LayoutManager#onFocusSearchFailed(View, int, Recycler, State)}方法去布局更多的视图
     * 在焦点查找的方向上
...
//        焦点查询算法类
        ...
        if (canRunFocusFailure
            ...
            if (mLayout.canScrollVertically()) {//垂直方向可以滑动
                final int absDir =
            ...
            //焦点查询失败
            if (!needsFocusFailureLayout && mLayout.canScrollHorizontally()) {//水平方向可以滑动
...
//                重新开始请求布局
    ...
    /**遍历给定视图的祖先并返回包含它的项目视图以及Recycler View的直接子项。 通过调用
     * {@link #get Child View Holder（View）}，可以使用此返回的视图来获取View Holder
     ...
     *包含给定视图的Recycler View的直接子节点，如果提供的视图不是此Recycler View的后代，则为null。
    ...
    public abstract static class LayoutManager {
        ...
        /**遍历给定视图的祖先并返回包含它的项目视图以及布局管理器的直接子项。
         ...
         * <p>请注意，如果视图是Recycler View的子级而不是布局管理器的子级（例如，运行消失动画），
         * 则此方法可能返回null。
         ...
         *布局管理器的直接子级，包含给定视图;如果提供的视图不是此布局管理器的后代，则为null。
